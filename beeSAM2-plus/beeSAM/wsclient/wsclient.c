...
void libwsclient_finish (wsclient *client)
{
	...
	{
		printf("[libwsclient_finish]Will join handshake_thread.\r\n");
		pthread_cancel(client->handshake_thread);
		printf("[libwsclient_finish]Have joined handshake_thread.\r\n");
	}

	if (client->helper_thread)
	{
		printf("[libwsclient_finish]Will kill helper_thread.\r\n");
		pthread_cancel(client->helper_thread);
		printf("[libwsclient_finish]Have killed helper_thread.\r\n");
	}

	if (client->run_thread)
	{
		printf("[libwsclient_finish]Will join run_thread.\r\n");
		pthread_cancel(client->run_thread);
		printf("[libwsclient_finish]Have joined run_thread.\r\n");
	}

	if (client->helper_sock)
	{
		close(client->helper_sock);
		printf("[libwsclient_finish]Have closed helper_sock.\r\n");
	}
	pthread_mutex_lock(&client->lock);
	client->helper_sock = 0;
	pthread_mutex_unlock(&client->lock);

	if (client->sockfd)
	{
		close(client->sockfd);
		printf("[libwsclient_finish]Have closed sockfd.\r\n");
	}
	pthread_mutex_lock(&client->lock);
	client->sockfd = 0;
	pthread_mutex_unlock(&client->lock);
...
void libwsclient_handle_control_frame (wsclient *c, wsclient_frame *ctl_frame)
{
	...
	case 0x8:
		...
		if ((c->flags & CLIENT_SENT_CLOSE_FRAME) == 0)
		{
			...
			for (i = 0; (unsigned long long) i < ctl_frame->payload_len; i++)
				*(ctl_frame->rawdata + ctl_frame->payload_offset + i) ^= (mask[i % 4] & 0xff); //mask payload
			*(ctl_frame->rawdata + 1) |= 0x80; //turn mask bit on
			i = 0;
			pthread_mutex_lock(&c->send_lock);
			while ((unsigned long long) i < ctl_frame->payload_offset + ctl_frame->payload_len && n >= 0)
			{
...
int libwsclient_helper_socket (wsclient *c, const char *path)
{
	...
	{
		printf("[libwsclient_helper_socket]Call bind() fail.\r\n");
	...
	{
		printf("[libwsclient_helper_socket]Call listen() fail.\r\n");
...
void *libwsclient_handshake_thread (void *ptr)
{
	...
	base64_encode((unsigned char*) key_nonce, 16, websocket_key, 256);
	...
	snprintf(request_headers, 1024,
			"GET /websocket HTTP/1.1\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nHost: %s\r\nSec-WebSocket-Key: %s\r\nSec-WebSocket-Version: 13\r\n\r\n",
			request_host, websocket_key);
	...
	SHA1Result(&shactx);
	memset(pre_encode, 0, 256);
	snprintf(pre_encode, 256, "%08x%08x%08x%08x%08x", shactx.Message_Digest[0], shactx.Message_Digest[1], shactx.Message_Digest[2],
			shactx.Message_Digest[3], shactx.Message_Digest[4]);
	for (z = 0; (size_t) z < (strlen(pre_encode) / 2); z++)
...
int libwsclient_send_fragment (wsclient *client, char *strdata, int len, int flags)
{
	...
	*(data + 1) = payload_len_small | 0x80; //payload length with mask bit on
	if (payload_len_small == 126)
	{
		payload_len &= 0xffff;
		len_size = 2;
		for (i = 0; (unsigned int) i < len_size; i++)
		{
...
int libwsclient_send (wsclient *client, char *strdata)
{
	...
	*(data + 1) = payload_len_small | 0x80; //payload length with mask bit on
	...
	for (i = 0; (size_t) i < strlen(strdata); i++)
		*(data + payload_offset + i) ^= mask[i % 4] & 0xff;
	sent = 0;
	i = 0;
